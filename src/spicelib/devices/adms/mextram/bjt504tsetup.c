...
int bjt504tsetup (SMPmatrix *matrix, GENmodel *inModel, CKTcircuit *ckt, int *states)
     /* load the bjt504t device structure with those pointers needed later 
      ...
      */
{
  ...
  /*  loop through all the bjt504t device models */
  for ( ;model != NULL ;model = model->bjt504tnextModel )
  {
    ...
    if(model->IZEB_Given == FALSE) model->IZEB=0.0;
    if(model->NZEB_Given == FALSE) model->NZEB=22.0;
    ...
    if(model->RCBLX_Given == FALSE) model->RCBLX=0.0;
    if(model->RCBLI_Given == FALSE) model->RCBLI=0.0;
    ...
    if(model->VGZEB_Given == FALSE) model->VGZEB=1.15;
    if(model->AVGEB_Given == FALSE) model->AVGEB=4.73e-4;
    if(model->TVGEB_Given == FALSE) model->TVGEB=636.0;
    ...
    if(model->ICSS_Given == FALSE) model->ICSS=(-1.0);
    ...
    if(model->ASUB_Given == FALSE) model->ASUB=2.0;
    ...
    if(model->GMIN_Given == FALSE) model->GMIN=1.0e-13;
    for ( here = model->bjt504tinstances ;here != NULL ; here = here->bjt504tnextInstance )
    {
      ...
      bjt504tguesstopology(matrix,ckt,model,here);
      ...
      /* Internal Nodes */
      {
        ...
        if(here->c1Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal collector node 1");
          if(error) return(error);
          here->c1Node = tmp->number;
        }
        if(here->e1Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal emitter node");
          if(error) return(error);
          here->e1Node = tmp->number;
        }
        if(here->b1Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal base node 1");
          if(error) return(error);
          here->b1Node = tmp->number;
        }
        if(here->b2Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal base node 2");
          if(error) return(error);
          here->b2Node = tmp->number;
        }
        if(here->c2Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal collector node 2");
          if(error) return(error);
          here->c2Node = tmp->number;
        }
        if(here->c3Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal collector node 3");
          if(error) return(error);
          here->c3Node = tmp->number;
        }
        if(here->c4Node == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal collector node 4");
          if(error) return(error);
          here->c4Node = tmp->number;
        }
        if(here->noiNode == -1)
        {
          error=CKTmkVolt(ckt,&tmp,here->bjt504tname,"internal noise node");
          if(error) return(error);
