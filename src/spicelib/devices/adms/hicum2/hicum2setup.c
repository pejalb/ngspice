      ...
      */
{
  ...
  for ( ;model != NULL ;model = model->hicum2nextModel )
  {
    ...
    if(model->vpt_Given == FALSE) model->vpt=100.0;
    ...
    for ( here = model->hicum2instances ;here != NULL ; here = here->hicum2nextInstance )
    {
      ...
      /* set states */
      here->state_xf_GND = *states; *states += 2;
      ...
      here->PTR_J_bi_xf_required=0;
      here->PTR_J_ei_xf_required=0;
      ...
      here->PTR_J_xf_bi_required=0;
      here->PTR_J_xf_ei_required=0;
      here->PTR_J_xf_ci_required=0;
      here->PTR_J_xf_tnode_required=0;
      here->PTR_J_xf_xf_required=0;
      ...
      /* Internal Nodes */
      {
        ...
        if(here->PTR_J_bi_xf_required==1)
        {
          here->PTR_J_bi_xf=SMPmakeElt(matrix,here->biNode,here->xfNode);
        }
        if(here->PTR_J_ei_xf_required==1)
        {
          here->PTR_J_ei_xf=SMPmakeElt(matrix,here->eiNode,here->xfNode);
        }
        ...
        if(here->PTR_J_xf_bi_required==1)
        {
          here->PTR_J_xf_bi=SMPmakeElt(matrix,here->xfNode,here->biNode);
        }
        if(here->PTR_J_xf_ei_required==1)
        {
          here->PTR_J_xf_ei=SMPmakeElt(matrix,here->xfNode,here->eiNode);
        }
        if(here->PTR_J_xf_ci_required==1)
        {
          here->PTR_J_xf_ci=SMPmakeElt(matrix,here->xfNode,here->ciNode);
        }
        if(here->PTR_J_xf_tnode_required==1)
        {
          here->PTR_J_xf_tnode=SMPmakeElt(matrix,here->xfNode,here->tnodeNode);
        }
        if(here->PTR_J_xf_xf_required==1)
        {
          here->PTR_J_xf_xf=SMPmakeElt(matrix,here->xfNode,here->xfNode);
        }
