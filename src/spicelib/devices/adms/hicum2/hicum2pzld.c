...
int hicum2pzLoad(GENmodel *inModel, CKTcircuit *ckt, SPcomplex *s)
{
  ...
  if(here->PTR_J_xf_bi_required) *(here->PTR_J_xf_bi)+=here->JSVAL_xf_bi *(s->real);
  if(here->PTR_J_xf_ei_required) *(here->PTR_J_xf_ei)+=here->JSVAL_xf_ei *(s->real);
  if(here->PTR_J_xf_ci_required) *(here->PTR_J_xf_ci)+=here->JSVAL_xf_ci *(s->real);
  if(here->PTR_J_xf_tnode_required) *(here->PTR_J_xf_tnode)+=here->JSVAL_xf_tnode *(s->real);
  if(here->PTR_J_xf_xf_required) *(here->PTR_J_xf_xf)+=here->JSVAL_xf_xf *(s->real);
  ...
  if(here->PTR_J_bi_xf_required) *(here->PTR_J_bi_xf)+=here->JDVAL_bi_xf *(s->real);
  if(here->PTR_J_ei_xf_required) *(here->PTR_J_ei_xf)+=here->JDVAL_ei_xf *(s->real);
  ...
  if(here->PTR_J_xf_xf_required) *(here->PTR_J_xf_xf)+=here->JDVAL_xf_xf *(s->real);
  ...
  if(here->PTR_J_bi_xf_required) *(here->PTR_J_bi_xf+1)+=here->JDVAL_bi_xf *(s->imag);
  if(here->PTR_J_ei_xf_required) *(here->PTR_J_ei_xf+1)+=here->JDVAL_ei_xf *(s->imag);
  ...
  if(here->PTR_J_xf_xf_required) *(here->PTR_J_xf_xf+1)+=here->JDVAL_xf_xf *(s->imag);
