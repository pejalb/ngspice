...
static void hicum2debug (CKTcircuit *ckt, hicum2model *model, hicum2instance *here)
{
  ...
  fprintf(stdout,"model: a_eg=%e\n",model->a_eg);
  fprintf(stdout,"model: ab=%e\n",model->ab);
  fprintf(stdout,"model: aa=%e\n",model->aa);
}
...
int hicum2load(GENmodel *inModel, CKTcircuit *ckt)
...
{  here->JSVAL_bi_bi=0.0;
  ...
  here->JSVAL_xf_bi=0.0;
  here->JSVAL_xf_ei=0.0;
  here->JSVAL_xf_ci=0.0;
  here->JSVAL_xf_tnode=0.0;
  here->JSVAL_xf_xf=0.0;
  ...
  here->JDVAL_bi_xf=0.0;
  here->JDVAL_ei_xf=0.0;
